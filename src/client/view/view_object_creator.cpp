...
const Player& ViewObjectCreator::createPlayer(size_t index, const Position& initial) const {
    Player* player = new Player(getSize(TYPE_PLAYER), initial, pixel_per_meter, textures["Player"]);
    view_objects[index] = player;
    return *player;
}

void ViewObjectCreator::createMetalBlock(size_t index, const Position& initial) const {
    MetalBlock* block = new MetalBlock(getSize(TYPE_METAL_BLOCK), initial, pixel_per_meter, textures["Block"]);
...
void ViewObjectCreator::createDiagonalMetalBlock(size_t index, const Position& initial, double rotation) const {
    DiagonalMetalBlock* block = new DiagonalMetalBlock(getSize(TYPE_METAL_DIAG_BLOCK), initial, pixel_per_meter, textures["Block"], rotation);
    view_objects[index] = block;
}

void ViewObjectCreator::createStoneBlock(size_t index, const Position& initial) const {
    StoneBlock* block = new StoneBlock(getSize(TYPE_STONE_BLOCK), initial, pixel_per_meter, textures["Block"]);
    view_objects[index] = block;
}

void ViewObjectCreator::createAcid(size_t index, const Position& initial) const {
    Acid* acid = new Acid(getSize(TYPE_ACID), initial, pixel_per_meter, textures["Miscellaneous"]);
    view_objects[index] = acid;
}

void ViewObjectCreator::createGate(size_t index, const Position& initial, double rotation) const {
    Gate* gate = new Gate(getSize(TYPE_GATE), initial, pixel_per_meter, textures["Gate"], rotation);
    view_objects[index] = gate;
}

void ViewObjectCreator::createButton(size_t index, const Position& initial) const {
    Button* button = new Button(getSize(TYPE_BUTTON), initial, pixel_per_meter, textures["Miscellaneous"]);
...
void ViewObjectCreator::createEnergyBall(size_t index, const Position& initial, double rotation) const {
    EnergyBall* energy_ball = new EnergyBall(getSize(TYPE_ENERGY_BALL), initial, pixel_per_meter, textures["FX"], rotation);
...
void ViewObjectCreator::createEnergyBar(size_t index, const Position& initial, double rotation) const {
    EnergyBar* energy_bar = new EnergyBar(getSize(TYPE_ENERGY_BAR), initial, pixel_per_meter, textures["FX"], rotation);
...
void ViewObjectCreator::createEndBarrier(size_t index, const Position& initial, double rotation) const {
    EndBarrier* energy_bar = new EndBarrier(getSize(TYPE_END_BARRIER), initial, pixel_per_meter, textures["FX"], rotation);
    view_objects[index] = energy_bar;
}

void ViewObjectCreator::createEnergyEmitter(size_t index, const Position& initial, double rotation) const {
    EnergyReceiver* energy_receiver = new EnergyReceiver(getSize(TYPE_ENERGY_EMITTER), initial, pixel_per_meter, textures["Block"], rotation);
...
void ViewObjectCreator::createEnergyReceiver(size_t index, const Position& initial, double rotation) const {
    EnergyReceiver* energy_receiver = new EnergyReceiver(getSize(TYPE_ENERGY_RECEIVER), initial, pixel_per_meter, textures["Block"], rotation);
    view_objects[index] = energy_receiver;
}

void ViewObjectCreator::createRock(size_t index, const Position& initial, double rotation) const {
    Rock* rock = new Rock(getSize(TYPE_ROCK), initial, pixel_per_meter, textures["Block"], rotation);
    view_objects[index] = rock;
}

void ViewObjectCreator::createPortal(size_t index, const Position& initial, double rotation) const {
    Portal* portal = new Portal(getSize(TYPE_PORTAL), initial, pixel_per_meter, textures["FX"], rotation);
